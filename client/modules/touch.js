...
;(function (global, factory) {
    ...
    jQuery.fn = jQuery.prototype = {
        ...
        // Start with an empty selectorch
        selector: '',
    ...
    jQuery.event = {
        ...
        trigger: function (event, data, elem, onlyHandlers) {
            ...
            while ((cur = eventPath[i++]) && !event.isPropagationStopped()) {
                ...
                // jQuery handler
                handle =
                    (data_priv.get(cur, 'events') || {})[event.type] && data_priv.get(cur, 'handle')
                if (handle) {
...
function TouchControls(valoria, options) {
    var self = this
    self.config = $.extend(
        {
            speedFactor: 0.5,
    ...
    .rotation-pad .region {
      ...
      width: 150px;
      height: 150px;
      background: radial-gradient(rgba(218, 225, 230, 0.25) 5%, rgba(218, 225, 230, 0.50) 95%)) center center no-repeat;
    ...
    parent.className = "touch-controls";
    valoria.el.appendChild(parent);
    ...
    this.avatar = valoria.avatar.model
    ...
    self.setAction = (toAction, timeScale) => {
    ...
    $(self.rotationPad).on('YawPitch', function (event) {
        ...
        self.rotateEnd.set(
            self.rotateStart.x + event.detail.deltaX,
            self.rotateStart.y - event.detail.deltaY
        )
        self.rotateDelta
    ...
    $(self.movementPad).on('move', function (event) {
        ...
        self.avatar.move['forward'] = event.detail.deltaY
        self.avatar.move['left'] = event.detail.deltaX

        // if (event.detail.deltaY == event.detail.middle) {
        // 	ztouch = 1;
        // 	moveForward = moveBackward = false;
        //   self.avatar.move["forward"] = 0;
        // } else {
        // 	if (event.detail.deltaY > event.detail.middle) {
        // 		moveForward = true;
        // 		moveBackward = false;
        //     self.avatar.move["forward"] = 1;
        // 	}
        ...
        //   self.avatar.move["left"] = 0;
        // } else {
        // 	if (event.detail.deltaX < event.detail.middle) {
        // 		moveRight = true;
        // 		moveLeft = false;
        //     self.avatar.move["left"] = -1;
        // 	}
    ...
    function calculateCameraRotation(dx, dy, factor) {
        ...
        var ry = self.avatar.rotation.y - dx * factor;
        var rx = self.camera.rotation.x - dy * factor;
    ...
    self.update = function (delta) {
        ...
        // self.avatar.translateX(velocity.x * -1 * self.speed);
        // self.avatar.translateY(velocity.y);
        // self.avatar.translateZ(velocity.z * -1 * self.speed);
        self.camera.dirTarget.position.set(
            self.camera.position.x + self.avatar.move.left * 10,
            self.camera.position.y,
            self.camera.position.z + self.avatar.move.forward * 20 - 2
        )
        var viewPos = self.camera.dirTarget.position
        var newView = new THREE.Vector3()
        newView.copy(viewPos)
        let pos = self.camera.dirTarget.getWorldPosition(newView)
        if (
            (self.avatar.move.forward !== 0 || self.avatar.move.left !== 0) &&
            (JSON.stringify(self.avatar.lastMove) !== JSON.stringify(self.avatar.move) ||
                JSON.stringify(self.camera.lastPosition) !== JSON.stringify(self.camera.position))
        ) { 
            self.scene.attach(self.camera)
            self.avatar.lookAt(
                self.avatar.position.x - (pos.x - self.avatar.position.x),
                self.avatar.position.y,
                self.avatar.position.z - (pos.z - self.avatar.position.z)
            )
            self.avatar.attach(self.camera)
        }
        ...
        offset.copy(position).sub(self.target) // rotate offset to "y-axis-is-up" space
        ...
        self.spherical.phi = Math.max(0.2, Math.min(Math.PI / 2 - 0.05, self.spherical.phi)) // restrict phi to be between desired limits
        self.spherical.makeSafe()
        self.spherical.radius *= 1 // restrict radius to be between desired limits
        self.spherical.radius = Math.max(0, Math.min(Infinity, self.spherical.radius)) // move target to panned location
        offset.setFromSpherical(self.spherical) // rotate offset back to "camera-up-vector-is-up" space
        offset.applyQuaternion(quatInverse);
        position.copy(self.target).add(offset)
        let isMoving = self.avatar.move.forward !== 0 || self.avatar.move.left !== 0
        if (self.avatar.jump) {
            self.setAction(self.avatar.animationActions["Jump"], 1)
        } else if (!self.avatar.jump) {
            if (isMoving) {
                self.setAction(self.avatar.animationActions["Run"], 1)
            } else {
                if (self.avatar.dancing) {
                    self.setAction(self.avatar.animationActions["Dance"], 1)
                } else {
                    self.setAction(self.avatar.animationActions["Idle"], 1)
                }
            }
        }
        let velocity = isMoving ? 1 : 0
        self.avatar.translateZ(velocity * self.speed * delta)
        self.avatar.lastMove = Object.assign({}, self.avatar.move)
        self.sphericalDelta.set(0, 0, 0)
        self.camera.lookAt(
            self.avatar.position.x,
            self.avatar.position.y + 1,
            self.avatar.position.z
        )
        self.camera.lastPos = Object.assign({}, self.camera.position)
    }
    ...
    self.hitTest = function () {
        ...
        for (var i = 0; i < 4; i++) {
            ...
            var rayCaster = new THREE.Raycaster(self.avatar.position, direction)
    ...
    self.getDirection2 = function (v) {
        ...
        var rx = self.camera.rotation.x
        var ry = self.avatar.rotation.y
...
TouchControls.prototype = {
    ...
    setRotation: function (x, y) {
        ...
        if (y !== null) this.avatar.rotation.y = y
    },
...
function RotationPad(container) {
    ...
    self.rotationPad.css({
        top:
            ...
            self.region.outerHeight() -
            10,
        left: self.container.width() - self.region.outerWidth() - 20,
    })
